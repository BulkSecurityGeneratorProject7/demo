...
export class EmployeeComponentsPage {
    ...
    deleteButtons = element.all(by.css('jhi-employee-job-order div table .btn-danger'));
    title = element.all(by.css('jhi-employee-job-order div h2#page-heading span')).first();
...
export class EmployeeUpdatePage {
    pageTitle = element(by.id('jhi-employee-job-order-heading'));
