...
import { LocationComponentsPage, LocationDeleteDialog, LocationUpdatePage } from './location-job-order.page-object';
...
describe('Location e2e test', () => {
    ...
    it('should load Locations', async () => {
        await navBarPage.goToEntity('location-job-order');
        locationComponentsPage = new LocationComponentsPage();
        expect(await locationComponentsPage.getTitle()).to.eq('prestozoomApp.location.home.title');
    });
    ...
    it('should load create Location page', async () => {
        ...
        expect(await locationUpdatePage.getPageTitle()).to.eq('prestozoomApp.location.home.createOrEditLabel');
    ...
    it('should create and save Locations', async () => {
        ...
        await promise.all([
            ...
            locationUpdatePage.setPhoneNumberInput('phoneNumber'),
            locationUpdatePage.companySelectLastOption()
        ]);
        ...
        expect(await locationUpdatePage.getPhoneNumberInput()).to.eq('phoneNumber');
    ...
    it('should delete last Location', async () => {
        ...
        expect(await locationDeleteDialog.getDialogTitle()).to.eq('prestozoomApp.location.delete.question');
