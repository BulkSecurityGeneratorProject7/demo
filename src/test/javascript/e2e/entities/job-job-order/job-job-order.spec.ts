...
import { JobComponentsPage, JobDeleteDialog, JobUpdatePage } from './job-job-order.page-object';
...
describe('Job e2e test', () => {
    ...
    it('should load Jobs', async () => {
        await navBarPage.goToEntity('job-job-order');
        jobComponentsPage = new JobComponentsPage();
        expect(await jobComponentsPage.getTitle()).to.eq('prestozoomApp.job.home.title');
    });

    it('should load create Job page', async () => {
        await jobComponentsPage.clickOnCreateButton();
        jobUpdatePage = new JobUpdatePage();
        expect(await jobUpdatePage.getPageTitle()).to.eq('prestozoomApp.job.home.createOrEditLabel');
    ...
    it('should delete last Job', async () => {
        ...
        expect(await jobDeleteDialog.getDialogTitle()).to.eq('prestozoomApp.job.delete.question');
