...
import { TaskComponentsPage, TaskDeleteDialog, TaskUpdatePage } from './task-job-order.page-object';
...
describe('Task e2e test', () => {
    ...
    it('should load Tasks', async () => {
        await navBarPage.goToEntity('task-job-order');
        taskComponentsPage = new TaskComponentsPage();
        expect(await taskComponentsPage.getTitle()).to.eq('prestozoomApp.task.home.title');
    });

    it('should load create Task page', async () => {
        await taskComponentsPage.clickOnCreateButton();
        taskUpdatePage = new TaskUpdatePage();
        expect(await taskUpdatePage.getPageTitle()).to.eq('prestozoomApp.task.home.createOrEditLabel');
    ...
    it('should delete last Task', async () => {
        ...
        expect(await taskDeleteDialog.getDialogTitle()).to.eq('prestozoomApp.task.delete.question');
